...
async function deleteBrief(id: string) {
  "use server";
  // use absolute URL to be safe on the server action too
  const base = getBaseUrl();
...
export default async function BriefsPage() {
  ...
  try {
    rows = await getBriefs();
  } catch (e: any) {
    error = e?.message || "Failed to load briefs.";
  }
    ...
    <div className="p-6 space-y-4">
      ...
      {error ? (
        ...
        <div className="text-zinc-400">
          No briefs yet. Create one from{" "}
          <Link className="underline" href="/brief/new">
            /brief/new
          </Link>
          .
        </div>
              ...
              <div key={r.id} className="rounded border border-zinc-800 p-4 flex flex-col gap-2">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <form action={async () => deleteBrief(r.id)}>
                    <button
                      type="submit"
                      className="px-3 py-1 rounded bg-red-700/80 hover:bg-red-600"
                    >
                      Delete
                    </button>
                  </form>
